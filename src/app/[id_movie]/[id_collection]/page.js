...
export default async function MovieCollectionPage({ params }) {
	...
	const data = await fetch(
		`https://api.themoviedb.org/3/collection/${id_collection}?api_key=${process.env.API_KEY}&language=es`
	);

	const res_collection_data = await data.json();
	...
	return (
		<>
			...
			<section className="flex flex-col gap-y-4 sm:max-w-screen-sm md:max-w-screen-md lg:max-w-screen-lg xl:max-w-screen-xl my-0m mx-auto">
				{parts.map((movie) => (
